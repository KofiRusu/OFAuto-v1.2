...
export async function POST(req: Request) {
  try {
    ...
    // Prepare subscription tiers if promotional details provided
    const subscriptionTiers = promoDetails ? [
      {
        tierId: "promo",
        price: promoDetails.price || price * (1 - (promoDetails.discountPercentage || 0) / 100),
        benefits: promoDetails.benefits || ["Promotional discount"]
      }
    ] : undefined;
    ...
    console.error("[ONLYFANS_PRICING_POST]", error);
    return new NextResponse(
